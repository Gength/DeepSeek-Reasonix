...
| `web_search` | true | Search the web using the current model provider's native search capability. Supports DeepSeek's built-in web_search tool (web_search_20250305). When the current provider is not DeepSeek, this tool returns a clear error suggesting alternatives (web_fetch or switching to a DeepSeek model). |
